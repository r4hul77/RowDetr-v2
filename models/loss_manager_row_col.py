...
class LossManager(nn.Module):
    ...
    def __init__(self, polyLossesWithCoeffs, classLossesWithCoeffs, disable_iou_loss=True) -> None: #lossesWithCoeffs is a dictionary containing the loss functions and their coefficients
        super().__init__()
        ...
        if not disable_iou_loss:
            self.IouAwareLoss = IOUAwareLoss()
        else:
            self.IouAwareLoss = None
    def forward(self, pred_polys, class_logits, targets, enc_polys=None, enc_class_logits=None, dn_polys=None, dn_logits=None, dn_queries=None):
        ...
        for i, matches in enumerate(idxs):
            ...
            for name, loss_coeff in self.polyLossesWithCoeffs.items():
                ...
                if(self.IouAwareLoss is not None):
