...
class VizImageHook(Hook):
    def __init__(self, interval=100, n_poly=2, n_degree=3, flip=False, **kwargs):
        ...
        for pnt in range(len(imgs)):
            ...
            for i in range(poly.shape[0]):
                if cls_logits[pnt, i] < 0.5:
                    ...
                    color = (0, 255, 0)
                x = np.polyval(poly[i, 0, :], alpha)*W
                if(self.flip):
                    y = np.clip(self.func(poly[i, 1, :], alpha)*H, 0, H)
                else:
                    y = np.polyval(poly[i, 1, :], alpha)*H
                for j in range(1, len(x)):
                    img = cv2.line(img, (int(x[j-1]), int(y[j-1])), (int(x[j]), int(y[j])), color, thickness)
    ...
    def draw_targets(self, imgs, targets, color, thickness):
        ...
        for i in range(len(imgs)):
            ...
            for i in range(no_polys):
                img = self.draw_poly(img, target[i, 0, :], target[i, 1, :], alpha, W, H, color, thickness)
            grid.append(img)
                        
        return grid
    
    def draw_poly(self, img, alpha_x, alpha_y, alpha, W, H, color, thickness):
        x = np.polyval(alpha_x, alpha)*W
        y = np.polyval(alpha_y, alpha)*H
        for j in range(1, len(x)):
            img = cv2.line(img, (int(x[j-1]), int(y[j-1])), (int(x[j]), int(y[j])), color, thickness) 
        return img